...
public class TypeExtender
{
    ...
    public void AddAttribute(Type type, object[] attributeCtorParams = null)
    {
        ...
        if (attributeCtorParams != null)
        {
            ...
            for (var index = 0; index < attributeCtorParams.Length; index++)
            {
        ...
        this._typeBuilder.SetCustomAttribute(attributeBuilder);
    }
    ...
    void addProperties(IEnumerable<string> properties, Type propertyType)
    {
        initializeTypeConstruction();
        foreach (var prop in properties)
        {
            var field = this._typeBuilder.DefineField($"_{prop}", propertyType, FieldAttributes.Private);
            var property = this._typeBuilder.DefineProperty(prop, PropertyAttributes.HasDefault, propertyType, null);
            generateGetter(prop, field, property, propertyType);
            generateSetter(prop, field, property, propertyType);
        }
    }

    void addField(string fieldName, Type type, Type attributeType, object[] attributeValues)
    {
        initializeTypeConstruction();
        var field = this._typeBuilder.DefineField(fieldName, type, FieldAttributes.Public);
        AddAttributeToField(attributeType, attributeValues, field);
    }

    void addField(string fieldName, Type type, Dictionary<Type, List<object>> attributeTypesAndParameters)
    {
        initializeTypeConstruction();
        var field = this._typeBuilder.DefineField(fieldName, type, FieldAttributes.Public);
    ...
    static void AddAttributeToField(Type attributeType, object[] attributeValues, FieldBuilder field)
    {
    ...
    void addProperty(string propertyName, Type type, Type attributeType, object[] attributeValues, bool isReadOnly)
    {
        addProperty(propertyName, type, new[] { new Tuple<Type, object[]>(attributeType, attributeValues) }, isReadOnly);
    }

    void addProperty(string propertyName, Type type, IEnumerable<Tuple<Type, object[]>> attributeTypesWithValues, bool isReadOnly)
    {
        initializeTypeConstruction();
        var field = this._typeBuilder.DefineField($"_{propertyName}", type, FieldAttributes.Private);
        var propertyBuilder = this._typeBuilder.DefineProperty(propertyName, PropertyAttributes.HasDefault, type, null);
    ...
    void addAttributeToProperty(Type attributeType, object[] attributeValues, PropertyBuilder propertyBuilder)
    {
        if (attributeType != null)
        {
            Type[] ctorArgsTypes;
    ...
    void generateSetter(string propertyName, FieldBuilder field, PropertyBuilder propertyBuilder, Type paramType)
    {
        var setMethodBuilder = this._typeBuilder.DefineMethod($"set_{propertyName}", this._getSetAttr, typeof(void), new[] { paramType });
    ...
    void generateGetter(string propertyName, FieldBuilder field, PropertyBuilder propertyBuilder, Type returnType)
    {
        var getMethodBuilder = this._typeBuilder.DefineMethod($"get_{propertyName}", this._getSetAttr, returnType, null);
    ...
    void initializeTypeConstruction()
    {
        ...
        if (this._typeBuilder != null && this._typeBuilder.IsCreated())
        {
            throw new ArgumentException($"The type {this._typeBuilder.Name} has already been created. You need to refresh the type extender class or create a new instance.");
        }
        ...
        this._assemblyName = new AssemblyName("DynamicAssembly");
        this._assemblyBuilder = AssemblyBuilder.DefineDynamicAssembly(this._assemblyName, AssemblyBuilderAccess.Run);
        this._moduleBuilder = this._assemblyBuilder.DefineDynamicModule(this._assemblyName.Name);
        this._typeBuilder = this._moduleBuilder.DefineType(TypeName, TypeAttributes.Public, BaseType);
    }
    ...
    readonly MethodAttributes _getSetAttr = MethodAttributes.Public | MethodAttributes.SpecialName | MethodAttributes.HideBySig;
}